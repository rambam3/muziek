...
<body class="bg-gray-100 text-gray-800">
    <div class="container mx-auto p-6">
        ...
        <h1 class="text-4xl font-bold text-center mb-10 text-gray-800">🎸 Band List</h1>
        
        <ul class="space-y-6">
            @foreach($bands as $index => $band)
            <li class="bg-white p-6 rounded-lg shadow flex items-center justify-between">
                <div>
                    <a href="{{ route('bands.show', $band->id) }}" class="text-xl font-semibold text-blue-600 hover:underline">{{ $band->name }}</a>
                </div>
        ...
        <div class="mt-12 text-center">
            <a href="{{ route('bands.create') }}" class="inline-block bg-green-500 text-white px-8 py-4 rounded-lg shadow hover:bg-green-600 transition focus:outline-none focus:ring-4 focus:ring-green-400">
                + Create a New Band
            </a>
        </div>
    </div>
